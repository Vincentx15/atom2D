...
class StatePoolLayer(pt.nn.Module):
    ...
    def forward(self, q, p, M):
        ...
        Ms = pt.nn.functional.softmax(self.sam.forward(z).unsqueeze(1) + F.unsqueeze(2), dim=0).view(M.shape[0],
                                                                                                     M.shape[1], -1, 2)
...
def get_top_k_batch(batch, k=64):
    with  pt.no_grad():
        graphs = batch.to_data_list()
        offset = 0
        all_top_k = []
        for graph in graphs:
            dists = pt.cdist(graph.pos, graph.pos)
            top_k = dists.topk(k=k).indices + offset
            offset += len(dists)
            all_top_k.append(top_k)
        all_top_k = pt.vstack(all_top_k)

    return all_top_k


class PestoModel(pt.nn.Module):
    ...
    def forward(self, graph, surface):
        ...
        ids_topk = get_top_k_batch(graph)
