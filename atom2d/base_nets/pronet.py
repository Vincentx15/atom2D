...
class ProNet(nn.Module):
    r"""
        ...
        Args:
            level: (str, optional): The level of protein representations. It could be :obj:`aminoacid`, obj:`backbone`,
                    and :obj:`allatom`. (default: :obj:`aminoacid`)
            ...
            data_augment_eachlayer (bool, optional): Data augmentation tricks. If set to :obj:`True`,
                                                     will add noise to the node features before each interaction block. (default: :obj:`False`)
    ...
    def forward(self, batch_data):
        # todo z need to be changed to the id of the amino acid
...
def associated_legendre_polynomials(L, zero_m_only=True, pos_m_only=True):
    ...
    z = sym.symbols("z")
    P_l_m = [[0] * (2 * ll + 1) for ll in range(L)]  # for order l: -l <= m <= l
    ...
    if L > 0:
        if zero_m_only:
            ...
            P_l_m[1][0] = z
            for ll in range(2, L):
                P_l_m[ll][0] = sym.simplify(
                    ((2 * ll - 1) * z * P_l_m[ll - 1][0] - (ll - 1) * P_l_m[ll - 2][0]) / ll
                )
            return P_l_m
        else:
            # for m >= 0
            for ll in range(1, L):
                P_l_m[ll][ll] = sym.simplify(
                    (1 - 2 * ll) * (1 - z ** 2) ** 0.5 * P_l_m[ll - 1][ll - 1]
                )  # P_00, P_11, P_22, P_33
            ...
            for ll in range(2, L):
                for m in range(ll - 1):  # P_20, P_30, P_31
                    P_l_m[ll][m] = sym.simplify(
                        (
                            (2 * ll - 1) * z * P_l_m[ll - 1][m]
                            - (ll + m - 1) * P_l_m[ll - 2][m]
                        )
                        / (ll - m)
                    )
            ...
            if not pos_m_only:
                ...
                for ll in range(1, L):
                    for m in range(1, ll + 1):  # P_1(-1), P_2(-1) P_2(-2)
                        P_l_m[ll][-m] = sym.simplify(
                            (-1) ** m
                            * np.math.factorial(ll - m)
                            / np.math.factorial(ll + m)
                            * P_l_m[ll][m]
                        )
...
def real_sph_harm(L, spherical_coordinates, zero_m_only=True):
    ...
    if zero_m_only:
        # for all m != 0: Y_lm = 0
        Y_l_m = [[0] for ll in range(L)]
    else:
        Y_l_m = [[0] * (2 * ll + 1) for ll in range(L)]  # for order l: -l <= m <= l
    ...
    if spherical_coordinates:
        ...
        for ll in range(L):
            for m in range(len(P_l_m[ll])):
                if not isinstance(P_l_m[ll][m], int):
                    P_l_m[ll][m] = P_l_m[ll][m].subs(z, sym.cos(theta))
    ...
    for ll in range(L):
        Y_l_m[ll][0] = sym.simplify(sph_harm_prefactor(ll, 0) * P_l_m[ll][0])  # Y_l0
    ...
    if not zero_m_only:
        ...
        for ll in range(1, L):
            ...
            for m in range(1, ll + 1):
                Y_l_m[ll][m] = sym.simplify(
                    2 ** 0.5
                    * (-1) ** m
                    * sph_harm_prefactor(ll, m)
                    * P_l_m[ll][m]
                    * sym.cos(m * phi)
                )
            # m < 0
        ...
        if not spherical_coordinates:
            ...
            y = sym.symbols("y")
            for ll in range(L):
                for m in range(len(Y_l_m[ll])):
                    Y_l_m[ll][m] = sym.simplify(Y_l_m[ll][m].subs(phi, sym.atan2(y, x)))
    return Y_l_m
...
class d_angle_emb(torch.nn.Module):
    def __init__(self, num_radial, num_spherical, cutoff=8.0):
        ...
        m = 0
        for ll in range(len(Y_lm)):
            if ll == 0:
                first_sph = sym.lambdify([theta], Y_lm[ll][m], modules)
                self.sph_funcs.append(
                    lambda theta: torch.zeros_like(theta) + first_sph(theta)
                )
            else:
                self.sph_funcs.append(sym.lambdify([theta], Y_lm[ll][m], modules))
            for n in range(num_radial):
                self.bessel_funcs.append(
                    sym.lambdify([x], bessel_formulas[ll][n], modules)
                )
...
class d_theta_phi_emb(torch.nn.Module):
    def __init__(self, num_radial, num_spherical, cutoff=8.0):
        ...
        modules = {"sin": torch.sin, "cos": torch.cos, "sqrt": torch.sqrt}
        for ll in range(len(Y_lm)):
            for m in range(len(Y_lm[ll])):
                if (
                        ll == 0
                ):
                    first_sph = sym.lambdify([theta, phi], Y_lm[ll][m], modules)
                    self.sph_funcs.append(lambda theta, phi: torch.zeros_like(theta) + first_sph(theta, phi))
                else:
                    self.sph_funcs.append(
                        sym.lambdify([theta, phi], Y_lm[ll][m], modules)
                    )
            for j in range(num_radial):
                self.bessel_funcs.append(
                    sym.lambdify([x], bessel_formulas[ll][j], modules)
                )
