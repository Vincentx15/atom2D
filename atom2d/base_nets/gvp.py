...
class GVPGNN(nn.Module):
    ...
    def __init__(self, node_in_dim, node_h_dim,
                 edge_in_dim=[16, 1], edge_h_dim=[16, 1], readout="mean",
        ...
        self.W_v = nn.Sequential(
            GVPLayerNorm(node_in_dim),
            GVP(node_in_dim, node_h_dim, activations=(None, None), vector_gate=vector_gate)
        )
        self.W_e = nn.Sequential(
            GVPLayerNorm(edge_in_dim),
            GVP(edge_in_dim, edge_h_dim, activations=(None, None), vector_gate=vector_gate)
        )

        self.layers = nn.ModuleList(GVPConvLayer(node_h_dim, edge_h_dim, drop_rate=drop_rate, activations=activations, vector_gate=vector_gate)
    ...
    def rbf(self, d, d_min=0.0, d_max=20.0):
        d_mu = torch.linspace(d_min, d_max, self.rbf_dim, device=d.device)
        d_mu = d_mu.view([1, -1])
    ...
    def forward(self, graph, *largs, **kwargs, ):
        ...
        node_position = graph.pos

        h_node = self.residue_embdding(input)

        node_in, node_out = edge_index
        pos_in, pos_out = node_position[node_in], node_position[node_out]
...
class GVP(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.input_dim_v:
            ...
            s = self.ws(x)
            if self.output_dim_v:
                v = torch.zeros(s.shape[0], self.output_dim_v, 3,
                                device=s.device)
        if self.scalar_act:
            s = self.scalar_act(s)
...
class GVPConvLayer(nn.Module):
    ...
    def __init__(self, node_dims, edge_dims,
        ...
        self.conv = GVPConv(node_dims, node_dims, edge_dims, n_message, aggr="add" if autoregressive else "mean",
